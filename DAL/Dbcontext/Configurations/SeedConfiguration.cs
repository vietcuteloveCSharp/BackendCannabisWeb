...
namespace DAL.Dbcontext.Configurations
{
	public class SeedConfiguration : IEntityTypeConfiguration<Seed>
	{
		public void Configure(EntityTypeBuilder<Seed> builder)
		{
			...
			builder.HasOne(c => c.Classification).WithMany(c => c.Seeds).HasForeignKey(c => c.ClassifyId).HasConstraintName("FK_SEED_CLASSIFICATION_CLASSIFYID").IsRequired();
			...
			builder.HasIndex(g => g.ProductId).HasDatabaseName("IX_Seed_ProductId");
		}
	}
}
